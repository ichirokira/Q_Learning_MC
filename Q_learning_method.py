...
def reward_function(network, q_learning, state, alpha=0.2, receive_func=find_receiver):
    ...
    w, nb_node_alive = get_weight(network, network.mc, q_learning, state, charging_time, receive_func)
    p_all = get_charge_per_sec_all(network, q_learning, state)
    ...
    time_move = distance.euclidean(q_learning.action_list[q_learning.state],
                                   q_learning.action_list[state]) / network.mc.velocity
    list_dead = []
    #w = [0 for _ in mc.list_request]
    for id in range(len(network.node)):
        temp = (network.node[id].energy - time_move * network.node[id].avg_energy) + (
                p_all[id] - network.node[id].avg_energy) * charging_time
        if temp < 0:
            list_dead.append(id)
    # p_hat = p / np.sum(p)
    ...
    second = ((len(network.node) - len(list_dead)) / len(network.node))

    # third = np.sum(w * p_hat)
    first = np.sum(e * p / E)

    return first, second, charging_time
...
def get_all_path(net, receive_func=find_receiver):
    list_path = []
    for sensor_id, sensor in enumerate(net.node):
...
def get_charge_per_sec_all(net, q_learning, state):
    return np.asarray(
        [para.alpha / (distance.euclidean(net.node[id].location,
                                          q_learning.action_list[state]) + para.beta) ** 2 for
         id in range(len(net.node))])
